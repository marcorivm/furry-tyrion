...
var TYRION = function(str, error_off, error_la) {
	...
	*/
	...
	return [__NODEJS_parse(str, error_off, error_la), TYRION.quads];
}



var parse = function(str, run) {
		var error_cnt = 0;
		var error_off = new Array();
		var error_la = new Array();
		error_cnt = TYRION(str, error_off, error_la);
		if(error_cnt[0] > 0) {
			for(var i = 0; i < error_cnt; i++ ) {
				IO.print( "Parse error near >" + str.substr( error_off[i], 30 ) + "<, expecting \"" + error_la[i].join() + "\"" );
			}
		} else {
			var quads = JSON.stringify(error_cnt[1]);
			if(run){
				FURRY(quads);
			} else {
				var a = document.createElement('a');
				a.href = 'data:Application/octet-stream,'+encodeURIComponent(quads);
